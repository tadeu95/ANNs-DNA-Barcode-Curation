...
df <- list(data.frame(North_Sea), data.frame(Norwegian_Sea), data.frame(Mediterranean_Sea), data.frame(Guinea_Current),
           ...
           data.frame(Celtic_Biscay_Shelf), data.frame(Faroe_Plateau), data.frame(Greenland_Sea), data.frame(North_Sea),
           data.frame(Iberian_Coastal), data.frame(Iceland_Shelf_and_Sea), data.frame(Namib), data.frame(Gulf_of_Guinea))
...
#Retrieving data from BOLD for Eastern Atlantic Marine fish using species list
...
#write_tsv(taxon,"taxon.tsv")
#taxon<-read.delim("taxon.tsv")
taxon2<-taxon[taxon$species_name!=""|is.na(taxon$species_name),]
taxon2<-taxon2[!(taxon2$bin_uri == "" | is.na(taxon2$bin_uri)), ]

#Retrieve records from all the previously mined BINs for comprehensivness
...
#Preprocess data and BAGS-like pipeline
...
# Add a new column with the count of sequences in each BIN
grade_e$count_in_cluster <- otu_counts[grade_e$BIN]
grade_e$count_in_cluster <- ave(seq_along(grade_e$BIN), grade_e$BIN, FUN = length)

#Calculate the number of sequences in a BIN, belonging to a particular species
...
# Calculate the percentage of sequences in BIN belonging to a particular species
...
grade_e5<-grade_e4[,c("processid","species","BIN","species_bin","species_per_bin","bin_per_species",
                      "country","percentage_sequences_in_cluster","identified_by","unique_identifiers","institution_storing","synonyms",
                      "frequency_species","base_number")]
...
# Function to filter synonyms based on their presence in the unique species
...
# Combine the filtered synonyms into a comma-separated string
...
#grade_e6$count_in_cluster <- otu_counts[grade_e6$BIN]
...
df_ordered <- bins_for_labelling[order(bins_for_labelling$BIN, bins_for_labelling$species), ]

write_tsv(df_ordered,"df_ordered.tsv")

#After labelling each record as reliable or unreliable in excel, read back the training set to convert the features for deep learning
...
#Go get labelled training set for Python
labelled_for_nn<-inner_join(excel[,c("processid","label")],grade_e12,by="processid")
...
#Create testing data set for labelling with ground truths

testing_df<-grade_e10[!(grade_e10$processid %in% bins_for_labelling$processid),]

write_tsv(testing_df,"testing_df_for_labelling.tsv")
